...
def get_mask_token_index(mask_token_id, inputs):
    ...
    # iterate through both the index and contents of list, as you need both
    for index, tokens_id in enumerate(input_ids):
        # compare each of the token id and check whether it matches the mask token id
        if tokens_id == mask_token_id:
            # if its a match, return the index of the token (location in the input sequence)
            return index
...
def get_color_for_attention_score(attention_score):
    ...
    """
    # used to convert a Tensorflow tensor to standard Python number, which is what we need in our calculations
